...
find_python() {
  local candidates=()
  if [[ -n "${CIVIC311_RELEASE_PYTHON:-}" ]]; then
    candidates+=("${CIVIC311_RELEASE_PYTHON}")
  fi
...
exec((root / "civic311" / "__init__.py").read_text(encoding="utf-8"), namespace)
...
dist = Path("dist")
wheel = dist / "civic311-0.1.1-py3-none-any.whl"
sdist = dist / "civic311-0.1.1.tar.gz"
